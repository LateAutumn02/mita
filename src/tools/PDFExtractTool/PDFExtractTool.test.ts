...
import { PDFExtractTool } from './PDFExtractTool.js'
...
test('PDFExtract extracts embedded text from a bounded page range', async () => {
  ...
  const result = await PDFExtractTool.call({
    pdf_path: pdfPath,
    start_page: 1,
    max_pages: 1,
    max_chars: 1000,
  ...
  expect(result.data.pages[0]?.text).toContain('Hello PDFExtract literature title')
...
test('PDFExtract reports out-of-range start pages without throwing', async () => {
  ...
  expect(result.data.warnings[0]).toContain('after the final page')
})
...
test('PDFExtract reports empty embedded text pages without OCR retrying', async () => {
  ...
  expect(result.data.warnings.some(warning => warning.includes('OCR is currently not enabled'))).toBe(true)
})
