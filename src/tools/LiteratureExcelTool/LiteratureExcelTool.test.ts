...
import { LiteratureExcelTool } from './LiteratureExcelTool.js'
...
test('LiteratureExcel writes generic JSON records to xlsx with requested columns', async () => {
  ...
  const result = await LiteratureExcelTool.call({
    columns,
    json_dir: dir,
    output_path: outputPath,
  })

  expect(result.data.rowsAdded).toBe(1)
  expect(result.data.invalidRecordsSkipped).toBe(0)
  expect(result.data.columns).toEqual(columns)
...
test('LiteratureExcel preserves existing rows and skips duplicates', async () => {
  ...
  await LiteratureExcelTool.call({
    columns,
  ...
  expect(result.data.existingRowsPreserved).toBe(1)
  expect(result.data.rowsAdded).toBe(1)
...
test('LiteratureExcel skips source-only empty extraction records', async () => {
  ...
  expect(result.data.rowsAdded).toBe(0)
  expect(result.data.invalidRecordsSkipped).toBe(1)
  expect(result.data.issues.some(issue => issue.message.includes('too few extracted fields'))).toBe(true)
})
